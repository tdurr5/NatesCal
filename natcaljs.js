...
function monthClick(e) {
    if ($(e).hasClass("clickable")) {
      ...
      var dateIndex = parseInt($(e).attr('id').split('-')[1]);
      startDate = new Date(currentYear, currentMonth, dateIndex);
      endDate = new Date(currentYear, currentMonth, dateIndex); // Set endDate to the same date as startDate
  
      $("#startdate").html(startDate.toString().split(' ').slice(0, 4).join(' '));
      $("#enddate").html(""); // Clear end date display
    }
  }
